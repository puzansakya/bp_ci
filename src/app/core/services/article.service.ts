...
export class ArticleService {
  ...
  getMyStoriesArticles(authorId: number): Observable<Articles> {
    return this.http
      .get<Articles>(`https://medium-puzan.herokuapp.com//api/v1/articles/mystories`)
      .pipe(catchError((error: any) => Observable.throw(error.json())));
  }
  ...
  updateArticle(article: Article): Observable<Article> {
    return this.http
      .put<Article>(`https://medium-puzan.herokuapp.com//api/v1/articles/${article.id}`, article)
      .pipe(catchError((error: any) => Observable.throw(error.json())));
  }
